...
int main(int argc, char *argv[]){
     ...
     */
     ...
     cloud_topic = priv_nh_.param<std::string>("cloud_topic", "vboats/cloud/raw");
     world_frame = priv_nh_.param<std::string>("world_frame", "base_link");
     camera_frame = priv_nh_.param<std::string>("camera_frame", "camera_depth_optical_frame");
     ...
     * SETUP PUBLISHERS
     */
     ros::Publisher object_pub, cluster_pub, pose_pub, plane_pub;
     ...
     plane_pub = nh.advertise<sensor_msgs::PointCloud2>("plane_cluster", 1);
     while (ros::ok()){
          /*
          ...
          * ========================================*/
          ...
          pcl::toROSMsg(*cloud_plane, *pc3_cloud);
          pc3_cloud->header.frame_id=world_frame;
          pc3_cloud->header.stamp=ros::Time::now();
          plane_pub.publish(pc3_cloud);

          /* ========================================
          * Fill Code: PUBLISH PLANE MARKER (OPTIONAL)
          * ========================================*/
          pcl::PointCloud<pcl::PointXYZ>::Ptr cropped_cloud2(new pcl::PointCloud<pcl::PointXYZ>(objects_cloud));
          pcl::SACSegmentation<pcl::PointXYZ> seg2;
          pcl::PointIndices::Ptr inliers2 (new pcl::PointIndices);
          pcl::ModelCoefficients::Ptr coefficients2 (new pcl::ModelCoefficients);
          seg2.setOptimizeCoefficients (true);
          // seg2.setModelType (pcl::SACMODEL_NORMAL_PARALLEL_PLANE);
          seg2.setModelType (pcl::SACMODEL_PARALLEL_PLANE);
          seg2.setMethodType (pcl::SAC_RANSAC);
          seg2.setMaxIterations (plane_max_iter);
          seg2.setDistanceThreshold (plane_dist_thresh);
          seg2.setAxis(Eigen::Vector3f(0, 0, -1));
          // seg2.setAxis(Eigen::Vector3f(0, -1, 0));
          // Segment the largest planar component from the cropped cloud
          seg2.setInputCloud (cropped_cloud2);
          seg2.segment (*inliers2, *coefficients2);

          // Extract the planar inliers from the input cloud
          pcl::ExtractIndices<pcl::PointXYZ> extract2;
          extract2.setInputCloud (cropped_cloud2);
          extract2.setIndices(inliers2);
          extract2.setNegative (false);

          // Get the points associated with the planar surface
          extract2.filter (*cloud_plane2);
          ROS_INFO_STREAM("PointCloud representing the planar component: " << cloud_plane2->points.size () << " data points." );
